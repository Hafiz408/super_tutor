...
- [ ] **UPLOAD-01**: User can upload a PDF file from the home page and create a study session from it
- [ ] **UPLOAD-02**: User can upload a Word (.docx) file from the home page and create a study session from it
- [ ] **UPLOAD-03**: Upload is a third selectable tab on the home page alongside URL and Topic
- [ ] **UPLOAD-04**: User can provide an optional focus prompt alongside the uploaded file (same as URL path)
...
- [ ] **SESSION-01**: Upload session produces the same notes, flashcards, and quiz output as URL and topic sessions
- [ ] **SESSION-02**: Upload session stores `session_type = "upload"` and filename as source in SQLite (same session_id contract)
- [ ] **SESSION-03**: User sees SSE progress events during upload session creation (same progress UX as URL and topic paths)
