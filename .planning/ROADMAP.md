...
**Goal**: Users can select a PDF or .docx file on the home page, validate it client-side, and create a study session from it with the same progress experience as URL and topic sessions
**Depends on**: Phase 12
**Requirements**: UPLOAD-01, UPLOAD-02, UPLOAD-03, UPLOAD-04, EXTRACT-03
**Success Criteria** (what must be TRUE):
  1. The home page shows a third "Upload" tab alongside URL and Topic; selecting it reveals a file input that accepts only `.pdf` and `.docx` files and an optional focus prompt field
  2. Selecting a file larger than 20 MB displays an inline error message before any network request is made; no request is sent to the backend
  3. After selecting a valid file and submitting, the user sees SSE progress messages (including an extraction step) and arrives at a complete study session page with notes, flashcards, and quiz working identically to other session types
  4. When the backend returns a scanned PDF error, the user sees a specific, actionable error message on the create page — not a generic error
  5. The frontend `fetch` call for upload never manually sets `Content-Type` — the browser sets it with the correct `multipart/form-data; boundary=` value automatically
**Plans**: TBD
